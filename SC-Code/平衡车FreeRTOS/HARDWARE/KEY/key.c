...
void KEY_NVIC_Init(void)
{
	  ...
	  NVIC_PriorityGroupConfig(NVIC_PriorityGroup_4);    //需要统一设置为4
		NVIC_InitStructure.NVIC_IRQChannel =EXTI4_IRQn;          
		NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 5;   //只能是5-15之间
...
{	
	delay_xms(20);   //注意这里的延时函数用不精确的ms和特制的delay_xms都可以，用delay_ms()会死机！！！
