...
{ 
	...
	if(n==1)
	{
	  speed_num1=Encoder_Value(TIM5); //读取XXms以后的编码器数值
		speed_num1=speed_num1*10;
		Encoder_Count_Clear(TIM5);       //将编码器清零，用于下次计数		
	}
	//2号电机测速
	if(n==2)
	{
	  speed_num2=-(Encoder_Value(TIM4));//读取XXms以后的编码器数值
		speed_num2=speed_num2*10;
		Encoder_Count_Clear(TIM4);      //将编码器清零，用于下次计数
	}	 
}
//****************速度1读取，带滤波****************************
void Speed1_Read(void)
{
  ...
  speed_avr1_num=Encoder_Value(TIM5); //读取XXms以后的编码器数值
...
void Speed2_Read(void)
{
  ...
  speed_avr2_num=-(Encoder_Value(TIM4)); //读取XXms以后的编码器数值
